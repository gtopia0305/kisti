...
2-2) 인터렉티브 작업제출을 이용한 실행

```
$ salloc --partition=cas_v100_4 -J debug --nodes=2 -n 4 --time=08:00:00 --gres=gpu:2 --comment=etc
...
1-3) 인터렉티브 작업제출을 이용한 실행

```
$ salloc --partition=cas_v100_4 -J debug --nodes=1 -n 4 --time=08:00:00 --gres=gpu:2 --comment=etc
$ module load gcc/8.3.0 cuda/10.0 cudampi/openmpi-3.1.0 python/3.7.1
...
[https://developer.nvidia.com/nccl/nccl-legacy-downloads](https://developer.nvidia.com/nccl/nccl-legacy-downloads)에서 설치를 원하는 버전 다운로드
...
2-4) 위의 Example 3을 이용한 2노드 8GPU 실행 예제
...
2-3) 배치 작업 제출 스크립트(1노드 4GPU)(tf\_dist\_run.sh)
...
3-3) 배치 작업 제출 스크립트(2노드 각4GPU) (tf\_multi\_run.sh)
...
4-2) \[텐서플로2] MNIST 데이터를 훈련 데이터로 사용한 DNN 학습([http://www.gisdeveloper.co.kr/?p=8534](http://www.gisdeveloper.co.kr/?p=8534))
...
[PytorchDDP(DistributedDataParallel)](https://pytorch.org/tutorials/intermediate/ddp\_tutorial.html)는 멀티 노드, 멀티GPU환경에서 실행할 수 있는 분산 데이터 병렬처리 기능을 제공한다. PytorchDDP 기능 및 튜토리얼은 아래 웹사이트를 참고 한다.

https://pytorch.org/tutorials/intermediate/ddp\_tutorial.html
